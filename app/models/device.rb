class Device < ActiveRecord::Base
  ...
  def self.send_ios_notifications(message,national)
    ...
    url = URI.parse("http://staging-unionconnect.herokuapp.com/api/v2/push_notifications?app_id=#{national.app_id}&message=#{message.content}&group_id=#{group_id}&sender_id=#{message.sender_id}&conversation_id=#{message.conversation_id}")
